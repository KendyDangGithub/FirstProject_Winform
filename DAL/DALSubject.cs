...
namespace DAL
{
    public class DALSubject
    {
        ...
        public int InsertSubject(string mahp, string tenhp, int sotc)
        {
            string query = "USP_INSERT_HOCPHAN @Mahp , @tenhp , @sotc";
            return DataProvider.Instance.ExecuteNonQuery(query, new object[] { mahp, tenhp, sotc });
        }
        public int UpdateSubject(string mahp, string tenhp, int sotc)
        {
            string query = "USP_UPDATE_HOCPHAN @Mahp , @tenhp , @sotc";
            return DataProvider.Instance.ExecuteNonQuery(query, new object[] { mahp, tenhp, sotc });
        }
